package com.edge.fintrack.portfolio.mfund;
...
public class AddMFundActivity extends AppCompatActivity implements View.OnClickListener {
    DatePickerDialog datePickerDialog;
    // Session Manager Class
    SessionManager session;
    private EditText et_invdate;
    private AutoCompleteTextView act_scheme;
    private TextView tv_accountHolder;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_add_mfund);
        act_scheme = (AutoCompleteTextView) findViewById(R.id.act_scheme);
        ...
        act_scheme.setThreshold(1);//will start working from first character
        // act_state.setAdapter(state_adapter);//setting the adapter data into the AutoCompleteTextView
        // et_state.setTextColor(Color.RED);
        act_scheme.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.et_invdate:
                ...
                // date picker dialog
                datePickerDialog = new DatePickerDialog(AddMFundActivity.this,
